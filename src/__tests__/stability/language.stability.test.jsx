...
describe('Language stability: Navbar', () => {
  ...
  test('Navbar shows UK flag icon by default (lang state starts as en)', () => {
    render(
      <LanguageProvider>
        <Navbar />
      </LanguageProvider>
    );
    // lang state defaults to 'en', so the img alt is 'English' (UK flag shown)
    const flagImg = document.querySelector('button.botonIdioma img');
    expect(flagImg).toBeInTheDocument();
    expect(flagImg).toHaveAttribute('alt', 'English');
  });
